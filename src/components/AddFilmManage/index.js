...
import { actFetchAddFilmManage, actFetchEditFilmManage } from "../../containers/admin/module/action";
...
const AddFilm = (props) => {
  ...
  const onFinish = (phim) => {
    ...
    if (phim.maPhim) {
      ...
      props.handleEditFilm(form_data);
    } else {
  ...
  return (
    ...
    <Form {...layout} form={form} initialValues={{ remember: true }} onFinish={onFinish} onFinishFailed={onFinishFailed} style={{ border: "1px solid gray", width: "50%", margin: "50px auto", boxShadow: "2px 2px 10px 2px lightgray" }} >
      ...
      <Form.Item name="tenPhim" label="Tên Phim" rules={[{ required: true, message: 'không được bỏ trống tên phim!' }]}>
        <Input placeholder="Nhập tên phim" />
      </Form.Item>
      <Form.Item name="biDanh" label="Bí danh" rules={[{ required: true, message: 'không được bỏ trống bí danh!' }]}>
        <Input placeholder="Nhập bí danh" />
      </Form.Item>
      <Form.Item name="trailer" label="Trailer" rules={[{ required: true, message: 'không được bỏ trống trailer!' }]} >
        <Input placeholder="Thêm trailer" />
      </Form.Item>
      <Form.Item label="Hình ảnh" name="hinhAnh" rules={[{ required: true, message: 'không được bỏ trống hình phim!' }]} >
      ...
      <Form.Item name="moTa" label="Mô tả" rules={[{ required: true, message: 'không được bỏ trống mô tả bô phim!' }]} >
        <Input placeholder="Nhập thông tin mô tả" />
      </Form.Item>

      <Form.Item name="ngayKhoiChieu" label="Ngày khởi chiếu" rules={[{ required: true, message: 'không được bỏ trống ngày khởi chiếu!' }]}>
